    ...
    while True:
        while True:
            try:
                ...
                print('X超时重试 ' + downloadPath + " " + fileUrl)
                DelateFile(fileUrl)
            except requests.exceptions.ConnectionError:
                print('X连接错误重试 ' + downloadPath + " " + fileUrl)
                DelateFile(fileUrl)
            except:
                traceback.print_exc()
                ErrorStop()
    ...
    for l_1 in version_list:
        # if l_1['type'] == 'release':    #判断是否为正式版
            ...
            if A_URL not in A_URL_List:
                ...
                print('-添加资源文件 URL: ' + str(A_URL))
    print('-资源文件去重完成, 一共需要运行' + str(len(A_URL_List)) + '个配置文件')
    print('-去重后资源文件 URL 列表: ' + str(A_URL_List))
    print('--开始下载文件')
